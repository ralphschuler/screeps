...
export function parseHarnessArgs(argv = process.argv.slice(2), env = process.env, roots = {}) {
  ...
  return {
    mode,
    durationMinutes: Number(args.get('durationMinutes') ?? (mode === 'long' ? 120 : 5)),
...
function createLogger(options) {
  return function log(message) {
    ...
    console.log(line);
    fs.appendFileSync(path.join(options.artifactsDir, 'harness.log'), `${line}\n`);
  };
}
...
function compose(options, log, ...composeArgs) {
  return runShell('docker', ['compose', '-f', 'docker-compose.ci.yml', '-p', options.projectName, ...composeArgs], options, log, {
...
function cliEval(options, command, timeoutMs = 30000) {
  return new Promise((resolve, reject) => {
    const socket = net.createConnection({ host: '127.0.0.1', port: options.cliPort });
    let output = '';
...
async function createApi(options, summary, timeoutMs = 300000) {
  ...
  while (Date.now() < deadline) {
    ...
    api.http.defaults.headers.common['X-Server-Password'] = options.serverPassword;
...
async function uploadBot(options, summary, api) {
  if (!fs.existsSync(options.botBundle)) throw new Error(`Bot bundle missing: ${options.botBundle}`);
...
async function collectLogs(options, summary, log) {
  try {
    const composeLogs = await runShell('docker', ['compose', '-f', 'docker-compose.ci.yml', '-p', options.projectName, 'logs', '--no-color'], options, log, { cwd: options.serverRoot, capture: true });
    let processLogs = { stdout: '', stderr: '' };
    try {
      processLogs = await runShell('docker', ['compose', '-f', 'docker-compose.ci.yml', '-p', options.projectName, 'exec', '-T', 'screeps', 'sh', '-lc', 'cat /screeps/logs/*.log 2>/dev/null || true'], options, log, { cwd: options.serverRoot, capture: true });
    } catch (error) {
...
async function writeSummary(options, summary, status) {
  ...
  fs.writeFileSync(path.join(options.artifactsDir, 'summary.json'), JSON.stringify(summary, null, 2));
  fs.writeFileSync(path.join(options.artifactsDir, 'summary.md'), `# Screeps ${options.mode} run\n\nStatus: ${status}\n\nStarted: ${summary.startedAt}\nFinished: ${summary.finishedAt}\n\nChecks:\n\n\`\`\`json\n${JSON.stringify(summary.checks, null, 2)}\n\`\`\`\n\nMetrics:\n\n\`\`\`json\n${JSON.stringify(summary.metrics, null, 2)}\n\`\`\`\n\nErrors:\n\n${summary.errors.map(e => `- ${e}`).join('\n') || '- none'}\n`);
}
...
export async function runPrivateServerTest(options = parseHarnessArgs()) {
  ...
  try {
    await runShell('npm', ['run', 'build:mod'], options, log, { cwd: options.repoRoot });
    await runShell('npm', ['run', 'build:bot'], options, log, { cwd: options.repoRoot });
    ...
    if (summary.metrics.taskBoardRooms === 0 && options.mode !== 'smoke') throw new Error('Memory.creepTaskBoard did not record room task boards');
    await writeSummary(options, summary, 'passed');
    return summary;
  } catch (error) {
    summary.errors.push(error.stack || error.message || String(error));
    try { await collectLogs(options, summary, log); } catch {}
    await writeSummary(options, summary, 'failed');
    throw error;
  } finally {
    await compose(options, log, 'down', '-v', '--remove-orphans').catch(error => {
