  ...
  return class LegacyScreepsApiAdapter {
    constructor(options) {
      ...
      this.raw = {
        ...
        user: {
          code: {
            get: (branch) => this.client.userCodeGet(branch),
...
const SCENARIO_SEED_CONFIRMATION_MARKER = "__PI_SCENARIO_SEED__";
const SCENARIO_SEED_CONFIRMATION_FILENAME = "scenario-seed-confirmation.json";
...
export function parseScenarioSeedConfirmationOutput(output) {
  const markerIndex = String(output ?? "").indexOf(SCENARIO_SEED_CONFIRMATION_MARKER);
  if (markerIndex < 0) return null;

  const afterMarker = String(output).slice(markerIndex + SCENARIO_SEED_CONFIRMATION_MARKER.length);
  const doneIndex = afterMarker.indexOf("__PI_CLI_DONE_");
  const candidate = (doneIndex >= 0 ? afterMarker.slice(0, doneIndex) : afterMarker).trim();
...
export function recordScenarioSeedConfirmation(options, summary, output) {
  const confirmation = parseScenarioSeedConfirmationOutput(output) ?? (
    String(output ?? "").includes("__PI_CLI_DONE_ERR__")
      ? {
          ok: false,
          scenarios: options.scenarios ?? [],
          requestedRoom: options.roomName ?? null,
          errorOutput: sanitizeScenarioSeedOutput(output),
        }
      : null
  );
...
function selectLatestRuntimeSummary(existing, incoming) {
  if (!incoming) return existing ?? null;
  if (!existing || (incoming.failed ?? 0) > 0) return incoming;
  if ((existing.failed ?? 0) > 0) return existing;

  const existingTick = Number(existing.tick);
  const incomingTick = Number(incoming.tick);
  const incomingIsStale = Number.isFinite(existingTick)
    && Number.isFinite(incomingTick)
    && incomingTick < existingTick;
...
export function inspectMemorySnapshot(memory, summary, now = new Date(), options = {}) {
  ...
  summary.metrics.screepsmodTesting = selectLatestRuntimeSummary(
    summary.metrics.screepsmodTesting,
    testSummary,
  );
  if (playerSummary) {
    summary.metrics.screepsmodTestingPlayer = selectLatestRuntimeSummary(
      summary.metrics.screepsmodTestingPlayer,
      playerSummary,
    );
  }
  if (backendSummary) {
    summary.metrics.screepsmodTestingBackend = selectLatestRuntimeSummary(
...
export function buildEnsureBotUserCommand(options) {
  return `Promise.resolve().then(async()=>{ const username=${JSON.stringify(options.username)}; const requestedRoom=${JSON.stringify(options.roomName)}; const password=${JSON.stringify(options.password)}; const SOURCE_ENERGY=1500; const ENERGY_REGEN_TIME=300; const SPAWN_ENERGY_START=300; const SPAWN_ENERGY_CAPACITY=300; const SPAWN_HITS=5000; const plainTerrain='0'.repeat(2500); const terrainForBootstrapRoom=()=>{ let terrain=plainTerrain; for (const [x,y] of [[10,10],[40,10],[25,40]]) { const index=y*50+x; terrain=terrain.slice(0,index)+'1'+terrain.slice(index+1); } return terrain; }; const toArray=async(result)=>Array.isArray(result)?result:(result&&result.toArray?await result.toArray():[]); const ensureBootstrapRoom=async(roomName)=>{ let room=await storage.db.rooms.findOne({_id:roomName}); if(!room){ await storage.db.rooms.insert({_id:roomName,status:'normal',sourceKeepers:false}); } else if(room.status!=='normal'){ await storage.db.rooms.update({_id:roomName},{$set:{status:'normal'}}); } const terrainValue=terrainForBootstrapRoom(); const terrainRecord=await storage.db['rooms.terrain'].findOne({room:roomName}); if(!terrainRecord){ await storage.db['rooms.terrain'].insert({room:roomName,terrain:terrainValue}); } else if(!terrainRecord.terrain){ await storage.db['rooms.terrain'].update({room:roomName},{$set:{terrain:terrainValue}}); } const roomControllers=await toArray(await storage.db['rooms.objects'].find({type:'controller',room:roomName})); let controller=roomControllers[0]; for(const duplicateController of roomControllers.slice(1)){ await storage.db['rooms.objects'].removeWhere({_id:duplicateController._id}); } if(!controller){ await storage.db['rooms.objects'].insert({room:roomName,type:'controller',x:25,y:40,level:0}); controller=await storage.db['rooms.objects'].findOne({type:'controller',room:roomName}); } const sources=await toArray(await storage.db['rooms.objects'].find({type:'source',room:roomName})); if(sources.length===0){ await storage.db['rooms.objects'].insert([[10,10],[40,10]].map(([x,y])=>({room:roomName,type:'source',x,y,energy:SOURCE_ENERGY,energyCapacity:SOURCE_ENERGY,ticksToRegeneration:ENERGY_REGEN_TIME}))); } await map.openRoom(roomName); await map.updateTerrainData(); return controller; }; const userHasOwnedRoom=async(targetUser)=>{ if(!targetUser || !targetUser._id) return false; const ownedController=await storage.db['rooms.objects'].findOne({type:'controller',$or:[{user:targetUser._id},{user:String(targetUser._id)}]}); return Boolean(ownedController); }; const ensureRoomSpawnForUser=async(targetUser,roomName)=>{ const userId=targetUser._id; const roomSpawns=await toArray(await storage.db['rooms.objects'].find({type:'spawn',room:roomName})); const existingSpawn=roomSpawns.find(spawn=>String(spawn.user)===String(userId)); for(const duplicateSpawn of roomSpawns){ if(!existingSpawn||String(duplicateSpawn._id)!==String(existingSpawn._id)){ await storage.db['rooms.objects'].removeWhere({_id:duplicateSpawn._id}); } } const spawnEnergyFields={energy:SPAWN_ENERGY_START,energyCapacity:SPAWN_ENERGY_CAPACITY,store:{energy:SPAWN_ENERGY_START},storeCapacityResource:{energy:SPAWN_ENERGY_CAPACITY},hits:SPAWN_HITS,hitsMax:SPAWN_HITS,spawning:null,notifyWhenAttacked:false}; if(!existingSpawn){ await storage.db['rooms.objects'].insert({type:'spawn',room:roomName,x:25,y:25,name:'Spawn1',user:userId,...spawnEnergyFields}); } else { await storage.db['rooms.objects'].update({_id:existingSpawn._id},{$set:{...spawnEnergyFields,user:userId}}); } const gameTime=typeof common!=='undefined'&&common.getGametime?await common.getGametime():0; const ownedController=await storage.db['rooms.objects'].findOne({type:'controller',room:roomName}); const controllerLevel=Math.max(1,Number(ownedController&&ownedController.level)||1); await storage.db['rooms.objects'].update({$and:[{room:roomName},{type:'controller'}]},{$set:{user:userId,level:controllerLevel,progress:0,downgradeTime:null,safeMode:gameTime+20000}}); await storage.db.rooms.update({_id:roomName},{$set:{invaderGoal:1000000}}); await storage.env.sadd(storage.env.keys.ACTIVE_ROOMS,roomName); }; let user=await storage.db.users.findOne({username}); const ownedController=user&&await storage.db['rooms.objects'].findOne({type:'controller',$or:[{user:user._id},{user:String(user._id)}]}); let spawnRoom=ownedController?.room||requestedRoom; let controller=ownedController||await storage.db['rooms.objects'].findOne({type:'controller',room:requestedRoom}); const requestedTerrain=await storage.db['rooms.terrain'].findOne({room:spawnRoom}); if(!controller||!requestedTerrain?.terrain){ controller=await ensureBootstrapRoom(spawnRoom); } const controllerOwnedByOther=controller?.user&&(!user||String(controller.user)!==String(user._id)); if(!controller||controllerOwnedByOther){ const controllersResult=await storage.db['rooms.objects'].find({type:'controller'}); const controllers=await toArray(controllersResult); const fallback=controllers.find(item=>item&&item.room&&!item.user); if(!fallback){ await storage.db['rooms.objects'].update({_id:controller._id},{$set:{user:null}}); spawnRoom=requestedRoom; controller=await ensureBootstrapRoom(spawnRoom); } else { spawnRoom=fallback.room; controller=await ensureBootstrapRoom(spawnRoom); } } else { await ensureBootstrapRoom(spawnRoom); } const userIdForName=name=>String(name).replace(/[^A-Za-z0-9_.-]/g,'_').slice(0,80)||'swarm-bot'; const ensureUserRecord=async()=>{ let found=await storage.db.users.findOne({username}); if(found) return found; await storage.db.users.insert({_id:userIdForName(username),username,usernameLower:String(username).toLowerCase(),cpu:100,gcl:1,active:0,cpuAvailable:10000,rooms:[spawnRoom]}); return storage.db.users.findOne({username}); }; if(!user){ user=await ensureUserRecord(); } if(!user) throw new Error('No user could be created for '+username); await ensureRoomSpawnForUser(user,spawnRoom); if(!(await userHasOwnedRoom(user))) throw new Error('No owned room could be assigned to user '+username); const userRooms=Array.from(new Set([...(Array.isArray(user.rooms)?user.rooms:[]),spawnRoom])); await storage.db.users.update({_id:user._id},{$set:{active:0,cpu:100,cpuAvailable:10000,rooms:userRooms,bot:username}}); await setPassword(username,password); const activeCode=await storage.db['users.code'].findOne({user:user._id,activeWorld:true}); if(!activeCode){ await storage.db['users.code'].insert({user:user._id,modules:{main:''},branch:'default',activeWorld:true,activeSim:true}); } const memoryKey=storage.env.keys.MEMORY+user._id; const rawMemory=await storage.env.get(memoryKey); let memoryValid=false; try{ memoryValid=Boolean(rawMemory&&typeof JSON.parse(rawMemory)==='object'); }catch(error){} if(!memoryValid){ await storage.env.set(memoryKey,'{}'); } }).then(()=>print('__PI_CLI_DONE_OK__')).catch(error=>print('__PI_CLI_DONE_ERR__',error.stack||error.message||String(error)))`;
}


export function buildSeedRuntimeScenariosCommand(options) {
  const scenarios = options.scenarios ?? [];
  return `Promise.resolve().then(async()=>{ const username=${JSON.stringify(options.username)}; const requestedRoom=${JSON.stringify(options.roomName)}; const scenarios=${JSON.stringify(scenarios)}; if(!scenarios.length) return; const SOURCE_ENERGY=1500; const ENERGY_REGEN_TIME=300; const plainTerrain='0'.repeat(2500); const toArray=async(result)=>Array.isArray(result)?result:(result&&result.toArray?await result.toArray():[]); const hasScenario=name=>scenarios.indexOf(name)>=0; const seedEvidence={scenarios,requestedRoom,rooms:{},hardInvader:null}; const ensureRoom=async(roomName)=>{ let room=await storage.db.rooms.findOne({_id:roomName}); if(!room){ await storage.db.rooms.insert({_id:roomName,status:'normal',sourceKeepers:false}); } else if(room.status!=='normal'){ await storage.db.rooms.update({_id:roomName},{$set:{status:'normal'}}); } const terrainRecord=await storage.db['rooms.terrain'].findOne({room:roomName}); if(!terrainRecord){ await storage.db['rooms.terrain'].insert({room:roomName,terrain:plainTerrain}); } else if(!terrainRecord.terrain){ await storage.db['rooms.terrain'].update({room:roomName},{$set:{terrain:plainTerrain}}); } const roomControllers=await toArray(await storage.db['rooms.objects'].find({type:'controller',room:roomName})); let controller=roomControllers[0]; for(const duplicateController of roomControllers.slice(1)){ await storage.db['rooms.objects'].removeWhere({_id:duplicateController._id}); } if(!controller){ await storage.db['rooms.objects'].insert({room:roomName,type:'controller',x:25,y:40,level:0}); controller=await storage.db['rooms.objects'].findOne({type:'controller',room:roomName}); } const sources=await toArray(await storage.db['rooms.objects'].find({type:'source',room:roomName})); if(sources.length===0){ await storage.db['rooms.objects'].insert([[10,10],[40,10]].map(([x,y])=>({room:roomName,type:'source',x,y,energy:SOURCE_ENERGY,energyCapacity:SOURCE_ENERGY,ticksToRegeneration:ENERGY_REGEN_TIME}))); } await map.openRoom(roomName); return controller; }; const ensureUser=async(name)=>{ let user=await storage.db.users.findOne({username:name}); if(!user){ await storage.db.users.insert({username:name}); user=await storage.db.users.findOne({username:name}); } return user; }; const upsertObject=async(query,doc)=>{ const existing=await storage.db['rooms.objects'].findOne(query); if(existing&&existing._id){ await storage.db['rooms.objects'].update({_id:existing._id},{$set:doc}); return existing._id; } await storage.db['rooms.objects'].insert(doc); const created=await storage.db['rooms.objects'].findOne(query); return created&&created._id; }; const botUser=await storage.db.users.findOne({username}); if(!botUser||!botUser._id) throw new Error('No bot user for runtime scenario seeding: '+username); const userId=botUser._id; const homeController=await storage.db['rooms.objects'].findOne({type:'controller',user:userId})||await ensureRoom(requestedRoom); const homeRoom=homeController.room||requestedRoom; await ensureRoom(homeRoom); const remoteRoom='W1N2'; const allianceRoom='W1N3'; const recoveryRoom='W1N4'; const recoverySupportRoom='W2N4'; if(hasScenario('construction-economy')){ await upsertObject({type:'constructionSite',room:homeRoom,x:26,y:25,user:userId},{type:'constructionSite',room:homeRoom,x:26,y:25,user:userId,structureType:'extension',progress:0,progressTotal:300}); } if(hasScenario('remote-mining')){ await ensureRoom(remoteRoom); } if(hasScenario('defense-hostile')){ const enemy=await ensureUser('ScenarioEnemy'); const enemyId=''+enemy._id; const defenseRoom=homeRoom; await ensureRoom(defenseRoom); await upsertObject({type:'creep',room:defenseRoom,name:'ScenarioEnemyAttacker'},{type:'creep',room:defenseRoom,x:24,y:25,name:'ScenarioEnemyAttacker',user:enemyId,body:[{type:'attack',hits:100},{type:'move',hits:100}],hits:200,hitsMax:200,fatigue:0,spawning:false,ticksToLive:1500,notifyWhenAttacked:false}); } if(hasScenario('spawnless-siege')){ const siegeEnemy=await ensureUser('ScenarioSpawnlessSiege'); const siegeEnemyId=''+siegeEnemy._id; await ensureRoom(recoveryRoom); await ensureRoom(recoverySupportRoom); await storage.db['rooms.objects'].update({$and:[{room:recoveryRoom},{type:'controller'}]},{$set:{user:userId,level:5,progress:0,downgradeTime:null,safeMode:null}}); await storage.db['rooms.objects'].update({$and:[{room:recoverySupportRoom},{type:'controller'}]},{$set:{user:userId,level:6,progress:0,downgradeTime:null,safeMode:null}}); await upsertObject({type:'spawn',room:recoverySupportRoom,user:userId},{type:'spawn',room:recoverySupportRoom,x:25,y:25,name:'SpawnRecoverySupport',user:userId,energy:300,energyCapacity:300,store:{energy:300},storeCapacityResource:{energy:300},hits:5000,hitsMax:5000,spawning:null,notifyWhenAttacked:false}); const recoverySupportExtensions=[[24,24],[25,24],[26,24],[24,26],[25,26],[26,26],[27,24],[27,25],[27,26],[23,24]]; for(const [x,y] of recoverySupportExtensions){ await upsertObject({type:'extension',room:recoverySupportRoom,user:userId,x,y},{type:'extension',room:recoverySupportRoom,x,y,user:userId,energy:50,energyCapacity:50,store:{energy:50},storeCapacityResource:{energy:50},hits:1000,hitsMax:1000,notifyWhenAttacked:false}); } const recoveryDefenders=[{name:'ScenarioRecoveryGuardA',room:homeRoom,role:'guard',x:23,y:25,bodyTypes:[...Array(5).fill('attack'),...Array(5).fill('move')]},{name:'ScenarioRecoveryGuardB',room:homeRoom,role:'guard',x:23,y:26,bodyTypes:[...Array(5).fill('attack'),...Array(5).fill('move')]},{name:'ScenarioRecoveryRangerA',room:recoverySupportRoom,role:'ranger',x:23,y:25,bodyTypes:[...Array(5).fill('ranged_attack'),...Array(5).fill('move')]},{name:'ScenarioRecoveryRangerB',room:recoverySupportRoom,role:'ranger',x:23,y:26,bodyTypes:[...Array(5).fill('ranged_attack'),...Array(5).fill('move')]},{name:'ScenarioRecoveryHealer',room:recoverySupportRoom,role:'healer',x:23,y:27,bodyTypes:[...Array(5).fill('heal'),...Array(5).fill('move')]}]; for(const defender of recoveryDefenders){ const body=defender.bodyTypes.map(type=>({type,hits:100})); await upsertObject({type:'creep',room:defender.room,name:defender.name},{type:'creep',room:defender.room,x:defender.x,y:defender.y,name:defender.name,user:userId,body,hits:body.length*100,hitsMax:body.length*100,fatigue:0,spawning:false,ticksToLive:5000,notifyWhenAttacked:false}); } await storage.db['rooms.objects'].removeWhere({$and:[{room:recoveryRoom},{type:'spawn'},{user:userId}]}); await storage.db['rooms.objects'].removeWhere({$and:[{room:recoveryRoom},{type:'tower'},{user:userId}]}); await upsertObject({type:'constructionSite',room:recoveryRoom,x:25,y:25,user:userId},{type:'constructionSite',room:recoveryRoom,x:25,y:25,user:userId,structureType:'spawn',progress:0,progressTotal:15000}); const siegeBodyTypes=[...Array(5).fill('attack'),...Array(5).fill('ranged_attack'),...Array(5).fill('heal'),...Array(15).fill('move')]; const siegeBody=siegeBodyTypes.map(type=>({type,hits:100})); await upsertObject({type:'creep',room:recoveryRoom,name:'ScenarioSpawnlessSiege'},{type:'creep',room:recoveryRoom,x:5,y:5,name:'ScenarioSpawnlessSiege',user:siegeEnemyId,body:siegeBody,hits:siegeBody.length*100,hitsMax:siegeBody.length*100,fatigue:0,spawning:false,ticksToLive:5000,notifyWhenAttacked:false}); await storage.db['rooms.objects'].update({$and:[{room:homeRoom},{type:'source'}]},{$set:{energy:0,ticksToRegeneration:9999}}); await storage.db['rooms.objects'].update({$and:[{room:homeRoom},{type:'spawn'},{user:userId}]},{$set:{energy:100,store:{energy:100}}}); await upsertObject({type:'terminal',room:homeRoom,user:userId},{type:'terminal',room:homeRoom,x:26,y:25,user:userId,store:{energy:6000},storeCapacityResource:{energy:300000},cooldown:0,hits:300000,hitsMax:300000,notifyWhenAttacked:false}); await storage.env.sadd(storage.env.keys.ACTIVE_ROOMS,recoveryRoom); await storage.env.sadd(storage.env.keys.ACTIVE_ROOMS,recoverySupportRoom); seedEvidence.spawnlessSiege={room:recoveryRoom,supportRoom:recoverySupportRoom,enemy:'ScenarioSpawnlessSiege',bodyParts:siegeBody.length,spawnSite:true,spawnCount:0,towerCount:0,supportSpawnCount:1,seededDefenders:recoveryDefenders.map(defender=>({name:defender.name,room:defender.room,role:defender.role})),homeTerminalEnergy:6000,homeSourceEnergy:0}; } if(hasScenario('defense-hard-invader')){ const hardEnemy=await ensureUser('ScenarioHardInvader'); const hardEnemyId=''+hardEnemy._id; const defenseRoom=homeRoom; await ensureRoom(defenseRoom); await storage.db['rooms.objects'].update({$and:[{room:defenseRoom},{type:'controller'}]},{$set:{user:userId,level:4,progress:0,downgradeTime:null,safeMode:null}}); const extensionPositions=[[24,24],[25,24],[26,24],[24,26],[25,26],[26,26],[27,24],[27,25],[27,26],[23,24]]; for(const [x,y] of extensionPositions){ await upsertObject({type:'extension',room:defenseRoom,user:userId,x,y},{type:'extension',room:defenseRoom,x,y,user:userId,energy:50,energyCapacity:50,store:{energy:50},storeCapacityResource:{energy:50},hits:1000,hitsMax:1000,notifyWhenAttacked:false}); } const hardBodyTypes=[...Array(5).fill('tough'),...Array(25).fill('ranged_attack'),...Array(10).fill('move'),...Array(10).fill('heal')]; const hardBody=hardBodyTypes.map(type=>({type,hits:100})); const hardInvaderObjectId=await upsertObject({type:'creep',room:defenseRoom,name:'ScenarioHardInvader'},{type:'creep',room:defenseRoom,x:5,y:5,name:'ScenarioHardInvader',user:hardEnemyId,body:hardBody,hits:hardBody.length*100,hitsMax:hardBody.length*100,fatigue:0,spawning:false,ticksToLive:5000,notifyWhenAttacked:false}); const hardInvaderObject=(hardInvaderObjectId?await storage.db['rooms.objects'].findOne({_id:hardInvaderObjectId}):null)||await storage.db['rooms.objects'].findOne({type:'creep',room:defenseRoom,name:'ScenarioHardInvader'}); const hardInvaderBody=(hardInvaderObject&&Array.isArray(hardInvaderObject.body)?hardInvaderObject.body:hardBody); seedEvidence.hardInvader={seeded:Boolean(hardInvaderObject),objectId:hardInvaderObject&&hardInvaderObject._id?''+hardInvaderObject._id:null,name:hardInvaderObject?.name??'ScenarioHardInvader',room:hardInvaderObject?.room??defenseRoom,user:hardInvaderObject?.user?''+hardInvaderObject.user:hardEnemyId,username:'ScenarioHardInvader',bodyParts:hardInvaderBody.length,bodyTypes:hardInvaderBody.map(part=>part&&part.type?part.type:part),hits:hardInvaderObject?.hits??hardBody.length*100,hitsMax:hardInvaderObject?.hitsMax??hardBody.length*100,ticksToLive:hardInvaderObject?.ticksToLive??5000,x:hardInvaderObject?.x??5,y:hardInvaderObject?.y??5,spawning:Boolean(hardInvaderObject?.spawning),seedQuery:{room:defenseRoom,name:'ScenarioHardInvader'}}; } if(hasScenario('nukerless-nuke')){ const nukeGameTime=typeof common!=='undefined'&&common.getGametime?await common.getGametime():0; await upsertObject({type:'nuke',room:homeRoom,x:25,y:25},{type:'nuke',room:homeRoom,x:25,y:25,landTime:nukeGameTime+5000,launchRoomName:'ScenarioNukeSource'}); } if(hasScenario('stacked-nukes')){ const nukeGameTime=typeof common!=='undefined'&&common.getGametime?await common.getGametime():0; await upsertObject({type:'nuke',room:homeRoom,x:25,y:25,launchRoomName:'ScenarioNukeSourceA'},{type:'nuke',room:homeRoom,x:25,y:25,landTime:nukeGameTime+5000,launchRoomName:'ScenarioNukeSourceA'}); await upsertObject({type:'nuke',room:homeRoom,x:25,y:25,launchRoomName:'ScenarioNukeSourceB'},{type:'nuke',room:homeRoom,x:25,y:25,landTime:nukeGameTime+5000,launchRoomName:'ScenarioNukeSourceB'}); } if(hasScenario('alliance-safety')){ const ally=await ensureUser('TooAngel'); const allyId=''+ally._id; await ensureRoom(allianceRoom); await upsertObject({type:'creep',room:allianceRoom,name:'TooAngelScenarioAlly'},{type:'creep',room:allianceRoom,x:25,y:25,name:'TooAngelScenarioAlly',user:allyId,body:[{type:'move',hits:100}],hits:100,hitsMax:100,fatigue:0,spawning:false,ticksToLive:1500,notifyWhenAttacked:false}); } if(hasScenario('link-network')){ await storage.db['rooms.objects'].update({$and:[{room:homeRoom},{type:'controller'}]},{$set:{user:userId,level:7,progress:0,downgradeTime:null}}); await upsertObject({type:'storage',room:homeRoom,user:userId},{type:'storage',room:homeRoom,x:24,y:25,user:userId,store:{energy:50000},storeCapacityResource:{energy:1000000},hits:1000000,hitsMax:1000000,notifyWhenAttacked:false}); } if(hasScenario('terminal-market-lab-economy')){ const economyRoom='W2N1'; await ensureRoom(economyRoom); await storage.db['rooms.objects'].update({$and:[{room:homeRoom},{type:'controller'}]},{$set:{user:userId,level:7,progress:0,downgradeTime:null}}); await storage.db['rooms.objects'].update({$and:[{room:economyRoom},{type:'controller'}]},{$set:{user:userId,level:6,progress:0,downgradeTime:null}}); await upsertObject({type:'storage',room:homeRoom,user:userId},{type:'storage',room:homeRoom,x:24,y:25,user:userId,store:{energy:200000,H:12000,O:12000},storeCapacityResource:{energy:1000000,H:1000000,O:1000000},hits:1000000,hitsMax:1000000,notifyWhenAttacked:false}); await upsertObject({type:'terminal',room:homeRoom,user:userId},{type:'terminal',room:homeRoom,x:25,y:26,user:userId,store:{energy:30000,H:6000,O:6000},storeCapacityResource:{energy:300000,H:300000,O:300000},cooldown:0,hits:300000,hitsMax:300000,notifyWhenAttacked:false}); await upsertObject({type:'terminal',room:economyRoom,user:userId},{type:'terminal',room:economyRoom,x:25,y:25,user:userId,store:{energy:5000},storeCapacityResource:{energy:300000},cooldown:0,hits:300000,hitsMax:300000,notifyWhenAttacked:false}); await upsertObject({type:'lab',room:homeRoom,user:userId,x:23,y:25},{type:'lab',room:homeRoom,x:23,y:25,user:userId,mineralType:'H',store:{H:2000},storeCapacityResource:{H:3000},cooldown:0,hits:500,hitsMax:500,notifyWhenAttacked:false}); await upsertObject({type:'lab',room:homeRoom,user:userId,x:23,y:26},{type:'lab',room:homeRoom,x:23,y:26,user:userId,mineralType:'O',store:{O:2000},storeCapacityResource:{O:3000},cooldown:0,hits:500,hitsMax:500,notifyWhenAttacked:false}); await upsertObject({type:'lab',room:homeRoom,user:userId,x:24,y:26},{type:'lab',room:homeRoom,x:24,y:26,user:userId,store:{},storeCapacityResource:{},cooldown:0,hits:500,hitsMax:500,notifyWhenAttacked:false}); await map.openRoom(economyRoom); } const gameTime=typeof common!=='undefined'&&common.getGametime?await common.getGametime():0; seedEvidence.seededAt=gameTime; seedEvidence.rooms={home:homeRoom,remote:remoteRoom,alliance:allianceRoom,recovery:recoveryRoom,recoverySupport:recoverySupportRoom,economy:'W2N1'}; const memoryKey=storage.env.keys.MEMORY+userId; const rawMemory=await storage.env.get(memoryKey); let memory={}; try{ memory=rawMemory?JSON.parse(rawMemory):{}; }catch(error){ memory={}; } if(hasScenario('spawnless-siege')){ memory.creeps=memory.creeps||{}; for(const defender of seedEvidence.spawnlessSiege?.seededDefenders||[]){ memory.creeps[defender.name]={...(memory.creeps[defender.name]||{}),role:defender.role,family:'military',homeRoom:defender.room,version:1}; } } memory.screepsmodTestingScenarios={names:scenarios,seededAt:gameTime,rooms:{home:homeRoom,remote:remoteRoom,alliance:allianceRoom,recovery:recoveryRoom,recoverySupport:recoverySupportRoom,economy:'W2N1'},hardInvader:hasScenario('defense-hard-invader')?(seedEvidence.hardInvader||{room:homeRoom,bodyParts:50}):undefined,spawnlessSiege:hasScenario('spawnless-siege')?{hostileSeeded:true,enemy:'ScenarioSpawnlessSiege'}:undefined}; await storage.env.set(memoryKey,JSON.stringify(memory)); await storage.env.set('screepsmodTestingScenarioSeed:'+userId,JSON.stringify(seedEvidence)); await map.updateTerrainData(); return seedEvidence; }).then(seedEvidence=>{ if(seedEvidence) print('__PI_SCENARIO_SEED__'+JSON.stringify(seedEvidence)); print('__PI_CLI_DONE_OK__'); }).catch(error=>print('__PI_CLI_DONE_ERR__',error.stack||error.message||String(error)))`;
}
...
async function seedRuntimeScenarios(options, summary) {
  ...
  recordScenarioSeedConfirmation(options, summary, output);
...
async function uploadBot(options, summary, api) {
  ...
  const uploaded = await api.raw.user.code.get("$activeWorld");
  if (uploaded?.modules?.main !== code)
    throw new Error("Bot upload read-back did not match the requested main module");
  summary.checks.botUploaded = true;
  summary.checks.botUploadVerified = true;
}

async function activateBotUser(options, summary) {
  const command = `Promise.resolve().then(async()=>{ const user=await storage.db.users.findOne({username:${JSON.stringify(options.username)}}); if(!user||!user._id) throw new Error('Bot user missing before activation'); await storage.db.users.update({_id:user._id},{$set:{active:10000}}); }).then(()=>print('__PI_CLI_DONE_OK__')).catch(error=>print('__PI_CLI_DONE_ERR__',error.stack||error.message||String(error)))`;
  const output = await cliEval(options, command);
  if (!output.includes("__PI_CLI_DONE_OK__") || output.includes("__PI_CLI_DONE_ERR__")) {
    throw new Error(`Bot activation failed. CLI output:\n${output}`);
  }
  summary.checks.botActivated = true;
}
...
function hasFreshDurableMergedEvidence(testSummary, warmupTicks) {
  const testSource = String(testSummary.source ?? "");
  if (testSource !== "screepsmod-testing-merged") return false;

  const playerSummary = testSummary.sources?.player;
  const backendSummary = testSummary.sources?.backend;
  if (!playerSummary || !backendSummary) return false;
  if (playerSummary.runtimeWarmed !== true || backendSummary.runtimeWarmed !== true) return false;
  if ((playerSummary.failed ?? 0) > 0 || (backendSummary.failed ?? 0) > 0) return false;
  if ((playerSummary.skipped ?? 0) > 0 || (backendSummary.skipped ?? 0) > 0) return false;
  if (backendSummary.diagnostics?.playerSandboxSummarySource !== "durable-env") return false;

  const mergedTick = Number(testSummary.tick);
  const playerTick = Number(playerSummary.tick);
  const backendTick = Number(backendSummary.tick);
  if (!Number.isFinite(mergedTick) || !Number.isFinite(playerTick) || !Number.isFinite(backendTick)) return false;
  return mergedTick >= playerTick
    && mergedTick >= backendTick
    && mergedTick - playerTick < warmupTicks
    && mergedTick - backendTick < warmupTicks;
}
...
export function hasSmokeValidationEvidence(summary, options = {}) {
  ...
  if (!hasFreshDurableMergedEvidence(testSummary, taskBoardWarmupTicks)) return false;
...
export function validateSmokeSummary(summary, options = {}) {
  ...
  if (ticksAdvanced >= taskBoardWarmupTicks && (testSummary.skipped ?? 0) > 0 && !isCpuBenchmark)
    throw new Error(`screepsmod-testing skipped ${testSummary.skipped} runtime checks after warmup`);
  if (mode === "smoke" && ticksAdvanced >= taskBoardWarmupTicks && runtimeWarmed === false)
    throw new Error(`screepsmod-testing runtime was not warmed after ${taskBoardWarmupTicks} ticks`);
  ...
  if (mode === "smoke" && ticksAdvanced >= taskBoardWarmupTicks && !hasFreshDurableMergedEvidence(testSummary, taskBoardWarmupTicks))
    throw new Error("screepsmod-testing missing fresh durable player/backend evidence after warmup");
}
...
export async function runPrivateServerTest(options = parseHarnessArgs()) {
  ...
  try {
    ...
    await activateBotUser(options, summary);
