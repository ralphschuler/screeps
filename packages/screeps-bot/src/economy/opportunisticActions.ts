...
export function opportunisticPickup(creep: Creep, primaryAction: CreepAction): CreepAction {
  ...
  const dropped = creep.pos.findInRange(FIND_DROPPED_RESOURCES, CONFIG.maxRange, {
    filter: (r: Resource) => r.resourceType === RESOURCE_ENERGY && r.amount >= CONFIG.minDroppedEnergy
  });
...
export function opportunisticRepair(creep: Creep, primaryAction: CreepAction): CreepAction {
  ...
  if (damaged.length > 0) {
    ...
    if (creep.pos.isNearTo(closest) && closest.hits < closest.hitsMax * 0.3) {
      logger.debug(
        `${creep.name} opportunistically repairing ${closest.structureType} at ${closest.pos} (${closest.hits}/${closest.hitsMax})`
      );
...
export function opportunisticTransfer(creep: Creep, primaryAction: CreepAction): CreepAction {
  ...
  const needsEnergy: (StructureSpawn | StructureExtension | StructureTower)[] = creep.pos.findInRange(
    FIND_MY_STRUCTURES,
    1,
    {
  ...
  if (needsEnergy.length > 0) {
    ...
    const prioritized = needsEnergy.sort((a, b) => {
      const priorityA = a.structureType === STRUCTURE_SPAWN ? 3 : a.structureType === STRUCTURE_EXTENSION ? 2 : 1;
      const priorityB = b.structureType === STRUCTURE_SPAWN ? 3 : b.structureType === STRUCTURE_EXTENSION ? 2 : 1;
