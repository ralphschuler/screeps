...
export function initializeCacheEvents(): void {
  ...
  eventBus.on(
    ...
    event => {
      ...
      if (invalidated > 0) {
        logger.debug(`Invalidated ${invalidated} cache entries for died creep ${event.creepName}`);
      }
    },
  ...
  eventBus.on(
    ...
    event => {
      ...
      logger.debug(`Invalidated caches for destroyed ${structureType} in ${roomName}`, {
  ...
  eventBus.on(
    ...
    event => {
      ...
      logger.debug(`Invalidated caches for completed ${structureType} in ${roomName}`, {
        meta: { invalidated, structureType, roomName }
      });
    },
  ...
  eventBus.on(
    ...
    event => {
      ...
      logger.debug(`Invalidated ${invalidated} cache entries for hostile in ${roomName}`, {
  ...
  eventBus.on(
    ...
    event => {
      ...
      logger.info(`Invalidated ${invalidated} cache entries for lost remote ${remoteRoom}`, {
  ...
  eventBus.on(
    ...
    event => {
      ...
      logger.info(`Invalidated ${invalidated} cache entries for RCL upgrade in ${roomName}`, {
...
export function invalidateRoomVisibility(roomName: string): void {
  ...
  logger.debug(`Invalidated ${invalidated} cache entries for room visibility change: ${roomName}`);
}
...
export function triggerCacheCleanup(): number {
  ...
  if (cleaned > 0) {
    logger.debug(`Manual cache cleanup removed ${cleaned} entries`);
  }
