...
export function getRoleCache<T = any>(creep: Creep, roleType: string, dataKey: string): T | undefined {
  const key = getCacheKey(creep.name, roleType, dataKey);
  return globalCache.get<T>(key, { namespace: NAMESPACE });
}

export function setRoleCache<T = any>(creep: Creep, roleType: string, dataKey: string, value: T, ttl?: number): void {
  const key = getCacheKey(creep.name, roleType, dataKey);
...
export function deleteRoleCache(creep: Creep, roleType: string, dataKey?: string): void {
  if (dataKey) {
    const key = getCacheKey(creep.name, roleType, dataKey);
...
export function hasRoleCache(creep: Creep, roleType: string, dataKey: string): boolean {
...
// Helper functions
export function getCachedRepairTarget(creep: Creep, computeFn: () => Structure | null): Structure | null {
...
export function getCachedBuildTarget(creep: Creep, computeFn: () => ConstructionSite | null): ConstructionSite | null {
...
export function getAssignedSource(creep: Creep, sourceId?: Id<Source>): Source | null {
...
export function getSourceContainer(creep: Creep, source: Source): StructureContainer | null {
...
export function getControllerEnergySource(
  ...
  const cachedId = getRoleCache<Id<StructureContainer | StructureLink>>(creep, "upgrader", cacheKey);
  ...
  const nearbyLinks = allLinks.filter(
    link => controller.pos.getRangeTo(link) <= 3 && link.store.getUsedCapacity(RESOURCE_ENERGY) > 0
  );
  ...
  const nearbyContainers = allContainers.filter(
    container => controller.pos.getRangeTo(container) <= 3 && container.store.getUsedCapacity(RESOURCE_ENERGY) > 0
  );
...
export function clearTargetCaches(creep: Creep, roleType: string, targetTypes: string[]): void {
