...
function evictLRU(cache: ObjectCacheStore): void {
  ...
  logger.warn(`Cache LRU eviction: removed ${entriesToRemove} entries`, {
    meta: {
...
export function getCacheStatistics(): CacheStatistics {
  ...
  if (cache.objects.size > MAX_CACHE_SIZE) {
    logger.warn(`Cache size exceeds warning threshold: ${cache.objects.size} entries`, {
      meta: {
        size: cache.objects.size,
        threshold: MAX_CACHE_SIZE
      }
    });
  }
...
export function warmCache(): void {
  ...
  logger.debug(`Cache warmed for ${Object.keys(Game.rooms).length} rooms`, {
    meta: {
      cpuUsed: cpuUsed.toFixed(3),
      cacheSize: getCacheStore().objects.size
    }
  });
}
