...
describe("Blueprint Selection and Validation", () => {
  ...
  describe("Blueprint import/export", () => {
    ...
    it("should reject JSON missing required fields", () => {
      const missingName = importBlueprint(
        JSON.stringify({ rcl: 1, anchor: { x: 25, y: 25 }, structures: [], roads: [], ramparts: [] })
      );
      assert.isNull(missingName, "Should reject blueprint without name");

      const missingRcl = importBlueprint(
        JSON.stringify({ name: "test", anchor: { x: 25, y: 25 }, structures: [], roads: [], ramparts: [] })
      );
      assert.isNull(missingRcl, "Should reject blueprint without RCL");

      const missingAnchor = importBlueprint(
        JSON.stringify({ name: "test", rcl: 1, structures: [], roads: [], ramparts: [] })
      );
  ...
  describe("RCL-based blueprint versioning", () => {
    ...
    it("should properly version spawns across RCLs", () => {
      ...
      const economicSpawns = ECONOMIC_MATURITY_BLUEPRINT.structures.filter(
  ...
  describe("Blueprint efficiency comparison", () => {
    it("should compare blueprints based on overall score", () => {
      ...
      const bunkerSpread = Math.max(...bunkerStructs.map(s => Math.abs(s.x)), ...bunkerStructs.map(s => Math.abs(s.y)));

      const spreadStructs = spread.structures;
      const spreadSpread = Math.max(...spreadStructs.map(s => Math.abs(s.x)), ...spreadStructs.map(s => Math.abs(s.y)));
