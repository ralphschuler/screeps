/**
 * MCP SDK Client integration tests for screeps-typescript-mcp server
 *
...
describe("MCP SDK Client Integration - screeps-typescript-mcp", () => {
  ...
  describe("tools/list", () => {
    it("should list all available tools via MCP SDK", async () => {
      ...
      expect(toolNames).toContain("screeps_types_search");
      expect(toolNames).toContain("screeps_types_get");
      expect(toolNames).toContain("screeps_types_list");
      expect(toolNames).toContain("screeps_types_related");
      expect(toolNames).toContain("screeps_types_by_file");
  ...
  describe("resources/list", () => {
    it("should list all available resources via MCP SDK", async () => {
      ...
      // Should contain screeps-types:// URI scheme resources
      const hasTypesResources = result.resources.some(r => r.uri.startsWith("screeps-types://"));
      expect(hasTypesResources).toBe(true);
  ...
  describe("Server protocol compliance", () => {
    ...
    it("should have proper tool schemas", async () => {
      ...
      const searchTool = result.tools.find(t => t.name === "screeps_types_search");
      expect(searchTool).toBeDefined();
      expect(searchTool?.description).toBeTruthy();
      expect(searchTool?.inputSchema).toBeDefined();

      const getTool = result.tools.find(t => t.name === "screeps_types_get");
