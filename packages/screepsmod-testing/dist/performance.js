...
class PerformanceAssert {
     ...
     */
    static async cpuBudget(fn, maxCPU, message) {
        const tracker = new CPUTracker();
        tracker.start();
        await fn();
        const used = tracker.stop();
        if (used > maxCPU) {
            throw new Error(message || `CPU budget exceeded: used ${used.toFixed(2)} > ${maxCPU}`);
        }
    }
    /**
     * Assert that a function completes within a certain time
     */
    static async timeLimit(fn, maxMs, message) {
        const start = Date.now();
        await fn();
        const duration = Date.now() - start;
        if (duration > maxMs) {
            throw new Error(message || `Time limit exceeded: took ${duration}ms > ${maxMs}ms`);
        }
    }
    /**
     * Assert that memory usage stays within a limit
     */
    static async memoryLimit(fn, maxBytes, message) {
        const tracker = new MemoryTracker();
        tracker.start();
        await fn();
        const used = tracker.stop();
        if (used > maxBytes) {
            throw new Error(message || `Memory limit exceeded: used ${used} bytes > ${maxBytes} bytes`);
        }
    }
}
