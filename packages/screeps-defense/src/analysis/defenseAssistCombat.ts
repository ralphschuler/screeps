...
function isActiveBodyPart(part: BodyPartConstant | BodyPartDefinition): boolean {
  return typeof part === "string" || part.hits > 0;
}
...
export function calculateCombatPower(parts: Array<BodyPartConstant | BodyPartDefinition>): CombatPower {
  ...
  for (const part of parts) {
    if (!isActiveBodyPart(part)) continue;
    partCount++;
  ...
  return { partCount, attack, ranged, heal, dismantle, score: attack + ranged + heal + dismantle + partCount * PART_SIZE_SCORE };
}
...
function generateRangerBodies(energyCapacity: number): BodyTemplate[] {
  ...
  for (let rangedParts = 1; rangedParts <= 25; rangedParts++) {
    ...
    addCandidate(candidates, orderedBody({ [TOUGH]: Math.ceil(rangedParts / 2), [RANGED_ATTACK]: rangedParts, [MOVE]: rangedParts }), energyCapacity);
  }
  return candidates;
}
...
function generateHealerBodies(energyCapacity: number): BodyTemplate[] {
  ...
  for (let healParts = 1; healParts <= 25; healParts++) {
    ...
    addCandidate(candidates, orderedBody({ [TOUGH]: Math.ceil(healParts / 2), [HEAL]: healParts, [MOVE]: healParts }), energyCapacity);
  }
  return candidates;
}
...
export function isDefenseAssistBodyStrongerThanThreat(parts: BodyPartConstant[], threat: CombatPower | null | undefined): boolean {
...
function isHardThreat(threat: CombatPower | null | undefined): boolean {
  return Boolean(threat && (threat.partCount >= HARD_THREAT_PART_THRESHOLD || threat.score >= HARD_THREAT_SCORE_THRESHOLD));
}

export function isDefenseAssistThreatProfileHard(threatProfile: DefenseAssistThreatProfile | null | undefined): boolean {
  return Boolean(threatProfile && (isHardThreat(threatProfile.strongest) || isHardThreat(threatProfile.total)));
}
...
export function buildDefenseAssistBody(
  ...
  if (strongestThreat && strongestThreat.score > 0) {
    const viableCandidates = isDefenseAssistThreatProfileHard(threatProfile)
...
export function calculateAggregateDefenseResponsePlan(
  ...
  if (getPlannedCount() === 0 && totalPower.score > targetScore) {
    return { counts, bodies, totalPower, healerFloor: 0, targetScore, capped: false };
  }
