...
/**
 * Room intel accessor interface
 * Allows threat predictor to update room intelligence without tight coupling
 */
export interface RoomIntelAccessor {
...
export class ThreatPredictor {
  ...
  private roomIntelAccessor: RoomIntelAccessor | undefined;

  public constructor(config: Partial<ThreatPredictorConfig> = {}, roomIntelAccessor?: RoomIntelAccessor) {
    this.config = { ...DEFAULT_CONFIG, ...config };
    this.roomIntelAccessor = roomIntelAccessor;
  }

  /**
   * Set room intel accessor
   */
  public setRoomIntelAccessor(accessor: RoomIntelAccessor): void {
    this.roomIntelAccessor = accessor;
  }
    ...
    for (const [owner, tracks] of hostilesByOwner) {
      ...
      for (const ownedRoom of ownedRooms) {
        ...
        if (threateningCount >= 2 && totalStrength >= this.config.minThreatStrength) {
          ...
          if (this.roomIntelAccessor) {
            this.roomIntelAccessor.updateThreatLevel(ownedRoom.name, 2);
          }
        }
      }
    }
