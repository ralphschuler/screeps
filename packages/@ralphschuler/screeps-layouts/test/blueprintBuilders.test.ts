...
} from "../src/blueprintBuilders";
import type { BlueprintPoint } from "../src/blueprints/types";
...
describe("Blueprint Builder Helpers", () => {
  ...
  describe("createSpawnRoadRing", () => {
    it("should create 8 road positions around spawn", () => {
      const center: BlueprintPoint = { x: 0, y: 0 };
    ...
    it("should create road ring at offset position", () => {
      const center: BlueprintPoint = { x: 5, y: -1 };
      const roads = createSpawnRoadRing(center);
  ...
  describe("createRadialRoads", () => {
    it("should create roads in specified directions", () => {
      ...
      // Verify north roads
      assert.exists(
        roads.find(r => r.x === 0 && r.y === -1),
        "Road at (0, -1)"
      );
      assert.exists(
        roads.find(r => r.x === 0 && r.y === -2),
        "Road at (0, -2)"
      );
      assert.exists(
        roads.find(r => r.x === 0 && r.y === -3),
        "Road at (0, -3)"
      );
  ...
  describe("createStructureProtection", () => {
    it("should create ramparts for specified structure types", () => {
      ...
      const ramparts = createStructureProtection(structures, [STRUCTURE_SPAWN, STRUCTURE_TOWER]);

      assert.equal(ramparts.length, 3, "Should create ramparts for 2 spawns + 1 tower");

      // Verify spawn ramparts
      assert.exists(
        ramparts.find(r => r.x === 0 && r.y === 0),
        "Rampart at spawn (0, 0)"
      );
      assert.exists(
        ramparts.find(r => r.x === -2 && r.y === 0),
        "Rampart at spawn (-2, 0)"
      );

      // Verify tower rampart
      assert.exists(
        ramparts.find(r => r.x === 2 && r.y === 2),
        "Rampart at tower (2, 2)"
      );

      // Verify extension not protected
      assert.notExists(
        ramparts.find(r => r.x === 4 && r.y === 4),
        "Extension should not have rampart"
      );
    });
