...
export interface BudgetDashboardOptions {
  ...
  /** Optional kernel instance for process information */
  kernel?: Kernel;
  /** Optional stats integration for budget info */
...
export function renderBudgetDashboard(options: BudgetDashboardOptions = {}): number {
  ...
  // Require kernel and stats for this visualization
  if (!options.kernel || !options.stats?.getAdaptiveBudgetInfo) {
  ...
  // CPU stats
  visual.text(
    `Allocated: ${totalAllocated.toFixed(2)} | Used: ${totalUsed.toFixed(2)} | Available: ${options.kernel.getRemainingCpu().toFixed(2)}`,
    x,
    currentY,
    {
      color: "#AAAAAA",
  ...
  if (options.showProcesses !== false) {
    ...
    const processesArray = options.kernel.getProcesses();
 ...
 * @param kernel - Optional kernel instance
 * @param stats - Optional stats integration
 * @returns CPU cost of rendering
 */
export function renderCompactBudgetStatus(roomName?: string, kernel?: Kernel, stats?: StatsIntegration): number {
  const startCpu = Game.cpu.getUsed();

  // Require kernel and stats
  if (!kernel || !stats?.getAdaptiveBudgetInfo) {
    return 0;
  }
