 ...
 * 
 * Provides optimized movement functions for remote mining creeps.
 */
export class RemoteMiningMovement {
  constructor(
  ...
  moveToWithRemoteCache(
    ...
    const cachedPath = this.remotePaths.getRemoteMiningPath(creep.pos, targetPos, routeType);
    
    if (cachedPath && cachedPath.length > 0) {
      // Cache hit - use moveByPath (very cheap, no pathfinding)
      this.logger.debug(`Using cached path for ${creep.name} (${routeType})`, {
      ...
      if (atFirstStep || adjacentToPath) {
        ...
        this.logger.debug(`moveByPath failed for ${creep.name}, falling back to moveTo`, {
    ...
    const moveToResult = this.moveTo(creep, target, {
      ...options,
  ...
  moveRemoteHarvesterToSource(
    ...
    return this.moveToWithRemoteCache(creep, source, "harvester", {
  ...
  moveRemoteHaulerToStorage(
    ...
    return this.moveToWithRemoteCache(creep, storage, "hauler", {
  ...
  moveRemoteHaulerToRemote(
    ...
    return this.moveToWithRemoteCache(creep, targetPos, "hauler", {
      visualizePathStyle: { stroke: "#ffffff" }
    });
  }
}
